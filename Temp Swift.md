...
Результат методов, работающих с БД лучше возвращать через клоужеры, поскольку операция может быть асинхронной и не известно когда она заверишится. При этом можно использовать объект Result

Context - это данные в оперативной памяти, для помещения их в БД их нужно сохранять

Урок 14 Хранение данных с Realm

- Разбор ДЗ с CoreData 
- Создание кастомного AlerController
- Установка Realm
- Обзор документации по Realm
- Создание алгоритма первоначального заполнения базы
- Просмотр локальной БД с помощью Realm studio
- Решение проблемы с откртием файлы БД
 
Realm - клосплатформенная база. Подходит для многих языков. Выкупили MongoDB

Realm позволяет хранить данные локально

Realm сразу создавалась для мобилок. CoreData например сначала создавалась для десктопа.

У Realm хорошая документация

Структуры не наследуются. Для Realm необходимо создавать классы, поскольку они наследуются от класс `Object`

Для того чтобы проверить, было ли первоначальное заполнение БД используется флаг в `UserDefaults`

Запись в БД может замораживать основной поток. Чтобы этого не было, Ефремов предлагает записывать в основном потоке но асинхронно
...
Читать из БД не обязательно асинхронно

У Realm файлы имеют расширения realm

Чтобы каждый раз не искать базу realm можно через поиск найти их расположения и потом сохранить этот запрос как виртуальную папку

**В типах значений допускается вызов инициализатора другими инициализаторами**, поскольку у структур нет наследования и процесс инициализации более простой. Этот процесс называется делегированием. **В классов такое делать нельзя.**
...
Чтобы в классах реализовать делегирование инициализаторов, используются вспомогательные инициализаторы, с ключевым словом `convenience`

**Конструкцию** `typealeas` **удобно использовать, когда описание типа занимает много места**, т.е. используется много символов. Например можно использовать для клоужеров:

```swift

typealeas Listener = (T) -> Void
var listener: Listener?

```

**В Swift есть несколько возможностей реализации реактирования программирования** с асинхронным кодом. Для упрощения процесса существует фреймворк Combine. По умолчанию входит в SwiftUI

В SwiftUI необязательно использовать протоколы как прокладки.

В декларативном подходе, в отличие от императивного, все зависит от свойств. Мы не указываем последовательность действий, как при императивном подходе, а меняем свойства, что находит отражения в UI.

В SwiftUI можно использовать панель свойств, чтобы управлять свойствами объекта, при этом все изменения отражаются в коде. Также панель можно сделать плавающей, нажва на CMD + ПКМ в окне просмотра.

Каждый модификатор вызывает новую копию объекта, с добавленным свойством, за которое он отвечает. Поскольку предыдущая копия не сохраняется, стек объектов не разрастается.

Библиотека объектов в SwiftUI поставляется со сниппетами. Тамже можно посмотреть модификаторы.

![](images/20220720113202.png)

В SUI все строится на стеках.

Из рекомендаций канала разработчиков IOS: хороший курс по SwiftUI

https://www.youtube.com/playlist?list=PLwvDm4VfkdphqETTBf-DdjCoAvhai1QpO

**Чтобы вынести элементы в отдельный View**, можно их поместить в стэк, поставить курсор на стэк, CMD + Click, _Extract Subview_

![](images/20220720113306.png)


State свойства всегда должны быть приватными и иметь значения по умолчанию, посокольку это первичный инициализируюий источник данных

Binding - это вторичные свойства

В SwiftUI интерфейс меняется, как только меняются значения переменных.

Чтобы вынести несколько одинаковых модификаторов, их можно применить к стеку

Viper

Архитектурный подход разработан при участии Роберта мартина. О нем упоминается в книге чистая архитектура

В Viper архитектуре название папок регламентирвано.
